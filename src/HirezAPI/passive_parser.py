...
class PassiveParser:
    ...
    YOUR_ABILITIES = (
        r"(your|your first|casting an|with an) (ability|abilities) (cast|heal|deal)*"
    )
    ...
    PROT_SHRED = (
        r"(decreases enemy|reduce your target\'s) (magical|physical) protection"
    )
    ...
    def parse(self, passive_string: str) -> Set[PassiveAttribute]:
        ...
        if (
            "healing" in passive_string
            and "increased" in passive_string
        ...
        if "currently on cooldown" not in passive_string and re.search(
            self.YOUR_ABILITIES, passive_string
        ):
        ...
        if (
            "stacks" in passive_string
            and "permanent" not in passive_string
            and "evolves" not in passive_string
        ):
        ...
        if (
            re.search(self.PCT_OF_TARGET_HEALTH, passive_string)
            or "of their current health" in passive_string
        ):
        ...
        if (
            "increased damage to jungle monsters" in passive_string
            or "jungle monsters and bosses take" in passive_string
            or "damage against jungle camps" in passive_string
        ):
        ...
        if "while over" in passive_string and "while under" in passive_string:
        ...
        if (
            "stuns" in passive_string
            or "slows them" in passive_string
            or "slowed" in passive_string
            or "slower" in passive_string
        ):
        ...
        if (
            "crowd control immunity" in passive_string
            or "immune to crowd control" in passive_string
        ):
            properties.add(PassiveAttribute.IMMUNE_TO_CC)

        if "ability heals" in passive_string or "heal an allied god" in passive_string:
        ...
        if (
            "bonus damage equal to" in passive_string
            or "bonus damage of" in passive_string
            or "magical power in damage" in passive_string
            or "of your physical power" in passive_string
        ):
        ...
        if (
            "killing an enemy god or minion" in passive_string
            or "1 stack for a minion kill" in passive_string
            or "1 stack per minion kill" in passive_string
            or "anything dies" in passive_string
        ):
            properties.add(PassiveAttribute.EVOLVES_WITH_MINION_KILLS)

        if (
            "your critical" in passive_string
            or "hitting an enemy god with a critical strike" in passive_string
            or "critical hits on enemy gods" in passive_string
        ):
