...
class ItemAttribute(Enum):
    ...
    def god_type(self) -> GodType:
        ...
        return (
            GodType.MAGICAL
            if "magical" in self.value
            else GodType.PHYSICAL
            if "physical" in self.value
            else None
        )
...
class ItemProperty:
    ...
    def from_json(obj):
        ...
        if value.endswith("%"):
            return ItemProperty(
                attribute, percent_value=float(value.replace("%", "")) / 100
            )
    ...
    def from_json(obj):
        ...
        if item.type is ItemType.ITEM:
            item.item_properties = [
                ItemProperty.from_json(val)
                for val in obj["ItemDescription"]["Menuitems"]
            ]
        return item
