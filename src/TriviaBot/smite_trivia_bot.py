...
async def smitetrivia(message, *args):
    ...
    for q in range(question_count):
        ...
        if category is None:
            category = random.choices(list(question_mapping.keys()), weights=[10, 1, 2])[0]
        ...
        def check(m):
            ...
            elif not correct and answer.replace("%", "").isdigit() and m.content.replace("%", "").isdigit() and answers[m.author]["answered"] < 2:
    ...
    if not was_stopped and bool(correct_answers):
        description = [f'**{idx + 1}**. _{(await bot.fetch_user(u[0])).display_name}_ (Score: **{u[1]}**) {"<:mleh:472905075208093717>" if idx == 0 else ""}' for idx, u in enumerate(sorted(correct_answers.items(), key=lambda i: i[1], reverse=True))]
...
async def scores(ctx):
    with open("scores.json", "r") as f:
        try:
            ...
            description = [f'**{idx + 1}**. _{(await bot.fetch_user(u[0])).display_name}_ (Score: **{u[1]}**) {"<:mleh:472905075208093717>" if idx == 0 else ""}' for idx, u in enumerate(current_scores)]
