...
class BaseCalculator:
    ...
    def basic_attack_damage(
        ...
        return (crit_bonus if is_crit else 1) * \
            progression * (base + (per_level * level) + (scaling * power))
    ...
    def damage_dealt(
        ...
        return ((100 * (damage - damage_red)) * (1 - damage_mit)) / \
            ((0 if is_true else BaseCalculator.protections(
                prots, red_pct, red_flat, pen_pct, pen_flat)) + 100)
    ...
    def add_or_set_stat(self, stat: ItemAttribute, value: float | _Penetration):
        if self.has_stat(stat):
            if stat in (ItemAttribute.MAGICAL_PENETRATION, ItemAttribute.PHYSICAL_PENETRATION):
                ...
                self.set_stat(
                    stat, _Penetration(pen.flat + value.flat, pen.percent + value.percent))
                return
...
class BuildStatCalculator:
    ...
    def calculate_item_stats(self, item: Item) -> _Stats:
        ...
        for prop in item.item_properties:
            if prop.attribute.god_type is not None and \
                    prop.attribute.god_type != self.god.god.type:
                continue
            if prop.attribute == ItemAttribute.ATTACK_SPEED \
            ...
            if prop.attribute in \
                ...
                if prop.attribute == ItemAttribute.PENETRATION:
                    stat = ItemAttribute.MAGICAL_PENETRATION \
                ...
                continue
            if prop.attribute == ItemAttribute.PHYSICAL_CRITICAL_STRIKE_CHANCE \
                    and self.god.god.type == GodType.PHYSICAL:
                item_stats.add_or_set_stat(ItemAttribute.CRITICAL_STRIKE_CHANCE, prop.percent_value)
            item_stats.add_or_set_stat(prop.attribute, prop.flat_value or prop.percent_value)
        return item_stats
                ...
                if stat in (ItemAttribute.MAGICAL_PENETRATION, ItemAttribute.PHYSICAL_PENETRATION):
                    if float(f'{value.percent:.2f}') > float(f'{cap:.2f}'):
                        if stat in stats.overcapped_stats:
                            ...
                            stats.overcapped_stats[stat] = _Penetration(0, value.percent)
                        stats.set_stat(stat, _Penetration(value.flat, cap))
                    continue
                if stat == ItemAttribute.COOLDOWN_REDUCTION \
    ...
    def calculate_god_build_stats(self) -> _Stats:
        ...
        if ItemAttribute.PROTECTIONS in stats.stats:
            ...
            stats.set_stat(
                ItemAttribute.PHYSICAL_PROTECTION, phys_prots + (phys_prots * multiplier))
        ...
        if self.god.god.id == GodId.OLORUN:
            ...
            crit_chance = (0.15 + (int((magic_power - 150) / 10) * 0.01)) \
                if magic_power >= 150 else 0
...
class DamageCalculator:
    ...
    def calculate_basic_damage_dealt(
            ...
            pct_red: float = 0,
            damage_mit: float = 0,
            crit_bonus: float = 1.75) -> Tuple[float, bool]:
        power_type = ItemAttribute.PHYSICAL_POWER \
        ...
        for item in defending_god.build:
            # Evolved Prophetic Cloak
            if item.id == 24172:
                total_prots = defending_stats.get_stat(ItemAttribute.MAGICAL_PROTECTION) + \
        ...
        if attacking_god.god.type == GodType.PHYSICAL or attacking_god.god.id == GodId.OLORUN:
            if attacking_stats.has_stat(ItemAttribute.CRITICAL_STRIKE_CHANCE):
                crit_chance = attacking_stats.get_stat(ItemAttribute.CRITICAL_STRIKE_CHANCE)
        ...
        if has_qins:
            ...
            qins_bonus = 0.03 if defending_health < 2000 else \
                max(0.035 + (((defending_health - 2000) / 250) * 0.005), 0.05)
        ...
        pen_type = ItemAttribute.PHYSICAL_PENETRATION \
            if attacking_god.god.type == GodType.PHYSICAL \
            else ItemAttribute.MAGICAL_PENETRATION

        attacking_god_penetration = attacking_stats.get_stat(pen_type) if \
            attacking_stats.has_stat(pen_type) else _Penetration(0, 0)
        ...
        total_damage_dealt = BaseCalculator.damage_dealt(
            ...
            damage_mit,
            defending_stats.get_stat(ItemAttribute.DAMAGE_REDUCTION) if \
                defending_stats.has_stat(ItemAttribute.DAMAGE_REDUCTION) else 0)
    ...
    def calculate_basic_ttk(
            attacking_god: GodBuild,
            defending_god: GodBuild,
            assume_item_passives_stacked: bool = False) -> float:
        attacking_god_stats = BuildStatCalculator(attacking_god).calculate_god_build_stats()
        defending_god_stats = BuildStatCalculator(defending_god).calculate_god_build_stats()
        ...
        for item in attacking_god.build:
            ...
            # Dominance
            if item.id == 19924:
                pen = attacking_god_stats.get_stat(ItemAttribute.PHYSICAL_PENETRATION) if \
                    attacking_god_stats.has_stat(ItemAttribute.PHYSICAL_PENETRATION) else \
                    _Penetration(0, 0)
        ...
        for item in defending_god.build:
            ...
            # Sentinel's Embrace
            if item.id == 19627:
                defending_god_stats.add_or_set_stat(ItemAttribute.MAGICAL_PROTECTION, 40)
                defending_god_stats.add_or_set_stat(ItemAttribute.PHYSICAL_PROTECTION, 40)

        seconds = 0
        defending_health = initial_health = defending_god_stats.get_stat(ItemAttribute.HEALTH)
        ...
        og_pen = attacking_god_stats.get_stat(ItemAttribute.PHYSICAL_PENETRATION) if \
            attacking_god_stats.has_stat(ItemAttribute.PHYSICAL_PENETRATION) else \
            _Penetration(0, 0)
        og_power = attacking_god_stats.get_stat(ItemAttribute.PHYSICAL_POWER) if \
            attacking_god.god.type == GodType.PHYSICAL else 0
        ...
        if ItemAttribute.ATTACK_SPEED in attacking_god_stats.overcapped_stats:
            base_attack_speed = attacking_god_stats.overcapped_stats[ItemAttribute.ATTACK_SPEED]
        ...
        while defending_health > 0:
            ...
            renewal_stacks[:] = [s for s in renewal_stacks if not expire_renewal_stack(s)]
            ...
            spectral_stacks[:] = [s for s in spectral_stacks if not expire_spectral_stack(s)]
            ...
            if has_berserkers and defending_health > initial_health / 2 \
            ...
            if has_mail_of_renewal and \
                    pre_fire_seconds >= renewal_exp and pre_fire_seconds >= renewal_cd:
                ...
                if len(renewal_stacks) == 5:
                    defending_health += defending_god_stats.get_stat(ItemAttribute.HEALTH) * 0.15
