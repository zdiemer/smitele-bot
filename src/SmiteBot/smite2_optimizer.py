...
# How much `item_value` counts for. Zero — the table is generated, checked in
# and loaded, and deliberately not used yet.
#
# It makes the only measure we have worse, at every weight tried: mean item
# overlap with the corpus goes 1.95 (off), 1.79 at 0.1, 1.80 at 0.2, 1.55 at
# 0.35, 1.57 at 0.5. That is not necessarily the table being wrong, and it is
# worth understanding before turning it on. The accuracy harness scores
# agreement with the items people *play*; the table measures which items *win*,
# and the two genuinely disagree — Rod of Tahuti is the most-picked item in the
# game and its measured lift is negative. Raising this would trade a build that
...
class Smite2BuildOptimizer:
    ...
    def empirical_value(self, item: Item) -> float:
        """What the corpus says this item is worth beyond its stat line.

        A static table, checked in and regenerated by hand — `/optimize` never
        reads an aggregate at runtime, which is what keeps it able to answer for
        a god nobody has played. See `src/tools/derive_item_value.py`.

        Two honest caveats. It is total win-rate lift rather than a residual, so
        it double-counts a little of what the stat lines already scored. And it
        disagrees with pick rate on purpose: Rod of Tahuti is the most-picked
        item in the game and measures *negative*, which is why the weight below
        is small enough to break ties rather than decide builds.
        """
        return item_value.SMITE2.get(item.name, 0.0)
    ...
    def score(self, items: Sequence[Item]) -> float:
        ...
        for item in items:
            total += EMPIRICAL_WEIGHT * self.empirical_value(item)
