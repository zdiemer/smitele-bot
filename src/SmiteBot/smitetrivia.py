    ...
    def check_guess(self, guess: str) -> bool:
        for answer in self.valid_answers:
            ...
            if not correct and not answer.replace('%', '').isdigit():
                ...
                correct = edit_distance.SequenceMatcher(a=answer, b=guess.lower()).distance() <= 2
            if correct:
                return True
        return False
...
class TriviaQuestion:
    ...
    def __init__(self, question: str, answer: str | TriviaAnswer, image_url: str = None):
        self.question = question
        self.answer = answer if isinstance(answer, TriviaAnswer) else TriviaAnswer([answer])
        self.id = uuid.uuid4()
    ...
    def answer_is_number(self) -> bool:
        return len(self.answer.valid_answers) == 1 and \
            all([a.isdigit() for a in self.answer.valid_answers])
...
class ItemQuestionGenerator:
    ...
    def __init_question_bank(self):
        ...
        self.__question_bank = {
            ...
            ItemType.ITEM: list(filter(lambda q: q is not None, [
                TriviaQuestion(
                ...
                TriviaQuestion(
                    ...
                    item.name
                ) if (item.passive is not None and item.passive != "") or (item.aura is not None and item.aura != "") else None,
                TriviaQuestion(
                    f'How much does it cost to upgrade **{self.__all_items[item.parent_item_id].name}** into **{item.name}**?',
                    f'{item.price}'
                ) if item.parent_item_id is not None and item.price > 0 else None,
                TriviaQuestion(
                    'What item is this?',
                    item.name,
                    item.icon_url
    ...
    def __generate_properties_questions(item: Item) -> List[TriviaQuestion]:
        ...
        matched_values = list(filter(
            lambda ip: ip.flat_value == prop.flat_value or \
                ip.percent_value == prop.percent_value, item.item_properties))
...
class GodQuestionGenerator:
    ...
    def __init_question_bank(self):
        ...
        self.__question_bank = [
            ...
            TriviaQuestion(
                f'What pantheon is **{god.name}** a part of?',
                god.pantheon
            ),
            TriviaQuestion(
                f'Which god has the title **{god.title}**?',
                god.name
            ),
            TriviaQuestion(
    ...
    def __generate_abilities_questions(god: God) -> List[TriviaQuestion]:
        ...
        cooldown_rank = random.randint(0, len(ability.cooldown_by_rank) - 1) \
            if any(ability.cooldown_by_rank) else None
        cost_rank = random.randint(0, len(ability.cost_by_rank) - 1) \
            if any(ability.cost_by_rank) else None
        ability_with_modifier = f'{int(ability.cost_by_rank[cost_rank])} {ability.cost_modifier}' \
            if ability.cost_modifier is not None else None

        pattern = re.compile(ability.name, re.IGNORECASE)

        return list(filter(lambda q: q is not None, [
            TriviaQuestion(
            ...
            TriviaQuestion(
                ...
                TriviaAnswer([f'{int(ability.cooldown_by_rank[cooldown_rank])}', f'{int(ability.cooldown_by_rank[cooldown_rank])} seconds']),
                ability.icon_url
            ) if cooldown_rank is not None else None,
            TriviaQuestion(
                f'What is the Mana (or Omi, Rage, etc.) cost for **{god.name}\'s** **{ability.name}** at **rank {cost_rank + 1}**?',
                TriviaAnswer(list(filter(lambda a: a is not None, [f'{int(ability.cost_by_rank[cost_rank])}', ability_with_modifier]))),
                ability.icon_url
            ) if cost_rank is not None else None,
        ]))
...
class SmiteTrivia(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, gods: Dict[GodId, God], items: Dict[int, Item]):
        self.__bot = bot
        self.__gods = gods
        self.__all_items = items
        ...
        self.__consumables = list(filter(lambda i: i.type == ItemType.CONSUMABLE, active_item_list))
        self.__items = list(filter(lambda i: i.type == ItemType.ITEM, active_item_list))
        self.__relics = list(filter(lambda i: i.type == ItemType.RELIC, active_item_list))
    ...
    def __check_message(self, message: discord.Message, attempted_answers: dict, question: TriviaQuestion):
        correct = False
        ...
        if message.author not in attempted_answers.keys():
            attempted_answers[message.author] = {
                "answered": 1,
                "warned": False
            }
        else:
        ...
        if not correct and question.answer_is_number() and message.content.replace("%", "").isdigit() and attempted_answers[message.author]["answered"] < 3:
        ...
        if attempted_answers[message.author]["answered"] >= 3 and not attempted_answers[message.author]["warned"]:
            loop = asyncio.get_running_loop()
            loop.create_task(message.channel.send(
                embed=discord.Embed(
    ...
    def __get_next_question(self, category: TriviaCategory = None) -> Tuple[discord.Embed, TriviaQuestion]:
        if category is None:
            category = random.choices(list(TriviaCategory), weights=[1, 10, 10, 2], k=1)[0]
        if category == TriviaCategory.CONSUMABLES:
            return ItemQuestionGenerator(
                random.choice(self.__consumables), self.__all_items).question
        if category == TriviaCategory.GODS:
            return GodQuestionGenerator(
                random.choice(list(self.__gods.values())), self.__gods).question
        if category == TriviaCategory.ITEMS:
            return ItemQuestionGenerator(
                random.choice(self.__items), self.__all_items).question
        if category == TriviaCategory.RELICS:
            return ItemQuestionGenerator(
                random.choice(self.__relics), self.__all_items).question
        raise ValueError
    ...
    async def __smitetrivia(self, message: discord.Message, *args: tuple):
        ...
        for current_question in range(question_count):
            ...
            embed.title = f'❔ _Question **{current_question+1}** of **{question_count}**_' if question_count > 1 else "❔ _Question_"
            embed.color = discord.Color.blue()
            embed.add_field(name="Time Remaining:", value="_20 seconds_")

            exp = time.time() + 20
            task = asyncio.get_running_loop().create_task(self.__countdown_loop(await message.channel.send(embed=embed), exp, embed))
            try:
                msg: discord.Message = await self.__bot.wait_for('message', check=lambda msg: self.__check_message(msg, answers, question), timeout=20)
