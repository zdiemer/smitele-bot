...
class SmiteleBot(commands.Bot):
    ...
    __get_base_smite_wiki = lambda _, name: f'https://smite.fandom.com/wiki/{name}_voicelines'

    # A helper lambda for constructing the item image file names used in constructing the build image
    __get_item_image_file = lambda _, item_id: f'item{item_id}.jpg'

    # A helper lambda for getting a time string in the format the Hirez API expects
    __get_time_string_utcnow = lambda _: str(datetime.utcnow().strftime('%Y%m%d%H%M%S'))
    ...
    async def smitele(self, message, *args):
        ...
        if args is not None:
            if len(args) > 0:
                await message.channel.send(embed=discord.Embed(color=discord.Color.red(), description='Invalid command! This bot accepts the command `$smitele` (or `$st`)'))
                return

        await message.channel.send(embed=discord.Embed(color=discord.Color.blue(), title='Smite-le Started!', description='Name the god given the clues! You\'ll have six attempts.'))
        ...
        r = requests.get(god['godCard_URL'])
        open(self.GOD_IMAGE_FILE, 'wb').write(r.content)
        ...
        def check(m):
            ...
            if m.content.startswith('$stop'):
                ...
                loop.create_task(message.channel.send(embed=discord.Embed(color=discord.Color.red(), title='Smite-le round canceled!')))
                raise StoppedError

            guess = unidecode(m.content).lower().replace('-', ' ')
            return guess == self.answer_god_name.lower() or editdistance.eval(guess, self.answer_god_name.lower()) <= 2
        ...
        open(self.SKIN_IMAGE_FILE, 'wb').write(r.content)

        # Cropping the skin image that we got randomly
        with Image.open(self.SKIN_IMAGE_FILE) as im:
            size = 180
            width, height = im.size
            left = random.randint(0, width - size)
            top = random.randint(0, height - size)
            crop_image = im.crop((left, top, left + size, top + size))
            crop_image.save(self.SKIN_CROP_IMAGE_FILE)

        with open(self.SKIN_CROP_IMAGE_FILE, 'rb') as f:
            desc = 'Name the god with this skin'
            if await self.__send_round_message(round_number=1, description=desc, check=check, message=message, file=f, file_name=self.SKIN_CROP_IMAGE_FILE):
                return

        # Round 2
        build = []

        # Hirez's route for getting recommended items is highly out of date, so we'll get a top Ranked Conquest player's build
        ...
        while len(build) == 0:
            ...
            match_history = self.__make_hirez_request('getmatchhistory', random_player['player_id'], include_language_code=False)
        ...
        for key in sorted(item_urls):
            ...
            file_name = self.__get_item_image_file(key)
            r = requests.get(item_urls[key])
            open(file_name, 'wb').write(r.content)
            try:
                # Sometimes we don't get a full build, so this sets the final size of the build image
                if key <= 2:
        ...
        # To be fancy, we'll give our output build image an alpha channel, in case some images are missing ;)
        output_image = Image.new('RGBA', (total_width, total_height), (250, 250, 250, 0))
        posX, posY = (0, 0)

        # Enumerating through the images that successfully saved and resized, then pasting them to our new canvas
        for idx, im in enumerate(images):
            output_image.paste(im, (posX, posY))
            if idx != 2:
                posX += im.size[0]
            if idx == 2:
                posX, posY = (0, im.size[1])
        output_image.save(self.BUILD_IMAGE_FILE)

        with open(self.BUILD_IMAGE_FILE, 'rb') as f:
            desc = 'Hint: A top-ranked player of this god recently used this build in Ranked Conquest.'
            if await self.__send_round_message(round_number=2, description=desc, check=check, message=message, file=f, file_name=self.BUILD_IMAGE_FILE):
                return
        ...
        while audio_src is None:
            ...
            try:
                # Not all skins have voiceline pages on the Smite wiki, so retry until we get one that works
                r = requests.get(self.__get_base_smite_wiki(page_name.replace(' ', '_')))
                smite_wiki = BeautifulSoup(r.content, 'html.parser')
                # BeautifulSoup is amazing
                audio_blocks = smite_wiki.find_all('audio')

                # Exclude the first voice line, which is line played when the god is locked in (says their name)
        ...
        open(self.VOICE_LINE_FILE, 'wb').write(r.content)
        ...
        if self.current_player.voice is not None:
            ...
            client.play(discord.FFmpegPCMAudio(source=self.VOICE_LINE_FILE), after=lambda _: asyncio.run_coroutine_threadsafe(coro=client.disconnect(), loop=client.loop).result())
        else:
            # Otherwise, just upload the voice file to Discord
            with open(self.VOICE_LINE_FILE, 'rb') as f:
                df = discord.File(f)
                await message.channel.send(file=df)
        
        if await self.__send_round_message(round_number=3, description='Whose voice line was that?', check=check, message=message):
            return
        ...
        while not saved_image:
            try:
                ...
                open(self.ABILITY_IMAGE_FILE, 'wb').write(r.content)
                ...
                pass
        with open(self.ABILITY_IMAGE_FILE, 'rb') as f:
            if await self.__send_round_message(round_number=4, description='Hint: Here\'s one of the god\'s abilities', check=check, message=message, file=f, file_name=self.ABILITY_IMAGE_FILE):
                return

        # Round 5
        if await self.__send_round_message(round_number=5, description=f'The god has this title:\n```{god["Title"]}```', check=check, message=message):
            return
        
        # Round 6
        with Image.open(self.GOD_IMAGE_FILE) as im:
            size = 180
            width, height = im.size
            left = random.randint(0, width - size)
            top = random.randint(0, height - size)
        ...
        with open(self.GOD_IMAGE_FILE, 'rb') as ans_f:
            with open(self.GOD_CROP_IMAGE_FILE, 'rb') as f:
                desc = 'Final hint: This is a crop of the god\'s base skin'
                if await self.__send_round_message(round_number=6, description=desc, check=check, message=message, file=f, file_name=self.GOD_CROP_IMAGE_FILE, last_round=True, answer_file=ans_f, answer_file_name=self.GOD_IMAGE_FILE):
                    return
    ...
    def __create_signature(self, route, time_string):
        input = f'{self.__config["hirezDevId"]}{route}{self.__config["hirezAuthKey"]}{time_string}'
        return hashlib.md5(input.encode()).hexdigest()

    # Helper function for sending the repeated round messages to Discord
    async def __send_round_message(self, round_number, description, message, check, file = None, file_name = None, last_round = False, answer_file = None, answer_file_name = None):
        ...
        async def send_incorrect(incorrect_desc):
            ...
            await message.channel.send(file=answer_image, embed=discord.Embed(color=discord.Color.red(), description=incorrect_desc))

        exp = time.time() + 20
        task = asyncio.get_running_loop().create_task(self.__countdown_loop(await message.channel.send(file=picture, embed=embed), exp, embed))
        try:
            while True:
                msg = await self.wait_for('message', timeout=20)
                if self.current_player != msg.author:
                    await message.channel.send(embed=discord.Embed(color=discord.Color.red(), description=f'🛑 The current player is **{self.current_player.display_name}**.'))
                    continue
                if check(msg):
                    answer_time = time.time() - (exp - 20)
                    task.cancel()
                    # These emojis are from my Discord server so I'll need to update these to be more universal. :D
                    ans_description = f'✅ Correct, **{msg.author.display_name}**! You got it in {round(answer_time)} seconds. The answer was **{self.answer_god_name}**. <:frogchamp:566686914858713108>'
                    
                    await message.channel.send(embed=discord.Embed(color=discord.Color.green(), description=ans_description))
                    return True
                else:
                    task.cancel()
    ...
    def __start_session(self):
        ...
        r = requests.get(f'{self.BASE_HIREZ_URL}/createsessionJson/{self.__config["hirezDevId"]}/{self.__create_signature("createsession", time_string)}/{time_string}')
        self.__session_id = r.json()['session_id']

        with open(self.SESSION_FILE, 'w') as f:
            f.write(self.__session_id)

    # Helper function for downloading the gods to JSON
    def __try_init_gods(self):
        with open(self.GODS_FILE, 'r+') as f:
            try:
                self.__gods = json.load(f)
            except:
                self.__gods = self.__make_hirez_request('getgods')
                json.dump(self.__gods, f)
    ...
    def __validate_session(self):
        if self.__session_id is None:
            return False
        time_string = self.__get_time_string_utcnow()
        r = requests.get(f'{self.BASE_HIREZ_URL}/testsessionJson/{self.__config["hirezDevId"]}/{self.__create_signature("testsession", time_string)}/{self.__session_id}/{time_string}')
        return r.status_code == 200 and 'This was a successful test' in r.json()
    ...
    def __make_hirez_request(self, route, argument = None, include_language_code = True):
        ...
        time_string = self.__get_time_string_utcnow()
        url = f'{self.BASE_HIREZ_URL}/{route}Json/{self.__config["hirezDevId"]}/{self.__create_signature(route, time_string)}/{self.__session_id}/{time_string}'
